# =============================================================================
# GSEA打分排序图（Hallmark NES 排序气泡散点）
# organized 版本：中文分节导航 + 线性脚本，不封装函数
# =============================================================================
# 与 code/original.R 的关系：
#   original.R 是作者原样脚本，含读补充表、筛 DEG、GSEA 和绘图。
#   本仓库没有 science.adk0775_data_s1.csv 与 Hallmark GMT，也未安装
#   clusterProfiler，因此本整理版只保留可执行的绘图层。
# 与 code/example.R 的差异（不改变几何映射）：
#   1. 增加中文分节导航；
#   2. 排版便于逐行审阅。
# 作者 ggplot 映射保持不变：点大小 = setSize/10，显著性 = alpha(-log10 p)，
# 填充固定为 #0000ff。这与上传参考图的颜色映射不一致，属已知差异。
# =============================================================================

library(ggplot2)
library(ggrepel)

# ---- 第一步：定位条目根目录 ------------------------------------------------
# 无论从仓库根还是从 code/ 调用，都按脚本自身位置找 data/。
args <- commandArgs(trailingOnly = FALSE)
file_arg <- sub("^--file=", "", args[grepl("^--file=", args)])
script_dir <- if (length(file_arg)) {
  dirname(normalizePath(file_arg, winslash = "/", mustWork = TRUE))
} else {
  normalizePath(getwd(), winslash = "/", mustWork = TRUE)
}
root <- if (basename(script_dir) == "code") {
  dirname(script_dir)
} else {
  script_dir
}

# ---- 第二步：读取通路级 GSEA 表 --------------------------------------------
# 合成示例表，不是论文补充表。列：ID, NES, setSize, pvalue。
# 更换自己的结果时，保持这四列，并注意作者原脚本假定恰好 49 行。
data <- read.csv(file.path(root, "data/example_gsea.csv"), stringsAsFactors = FALSE)
data$setSize_1 <- data$setSize / 10
data <- data[order(data$NES, decreasing = TRUE), ]
data$ID <- factor(data$ID, levels = data$ID)
data$xlab <- seq_len(nrow(data))
if (nrow(data) != 49) {
  warning("作者原脚本把 xlab 写成 1:49；当前表有 ", nrow(data), " 行。")
}

# ---- 第三步：准备要标注的通路 ----------------------------------------------
# 向量里保留作者原有的 EMT；合成表和上传图都没有 EMT，因此只会标出 9 个。
# 9 个颜色按 NES 降序后的命中顺序一一对应，不要改顺序。
label <- c(
  "KRAS_SIGNALING_DN", "INTERFERON_ALPHA_RESPONSE",
  "UV_RESPONSE_DN", "EMT", "TNFA_SIGNALING_VIA_NFKB", "MYC_TARGETS_V2",
  "KRAS_SIGNALING_UP", "MYC_TARGETS_V1", "G2M_CHECKPOINT", "E2F_TARGETS"
)
data_label <- data[data$ID %in% label, ]
data_label$col <- c(
  "black", "grey80", "grey80", "grey80", "grey80",
  "#ffb882", "grey80", "#ff5eff", "#ff5eff"
)

# ---- 第四步：按作者映射画气泡散点 ------------------------------------------
# Detection 图例对应 size；Significance 图例对应 alpha，不是颜色。
p <- ggplot(data = data, aes(x = xlab, y = NES)) +
  geom_point(
    aes(size = setSize_1, alpha = -log10(pvalue)),
    shape = 21,
    stroke = 0.7,
    fill = "#0000ff",
    colour = "black"
  ) +
  scale_size_continuous(range = c(0.2, 8)) +
  xlab(label = "Hallmark gene sets") +
  ylab(label = "Normalized enrichment score (NES)") +
  theme_classic(base_size = 15) +
  scale_x_continuous(
    breaks = seq(0, 50, by = 10),
    labels = seq(0, 50, by = 10)
  ) +
  scale_y_continuous(
    breaks = seq(-4, 2.3, by = 1),
    labels = seq(-4, 2.3, by = 1)
  ) +
  guides(
    size = guide_legend(title = "Detection\n(proportion)"),
    alpha = guide_legend(title = "Significance\n(-log10 p-val.)")
  ) +
  theme(
    axis.line = element_line(color = "black", linewidth = 0.6),
    axis.text = element_text(face = "bold"),
    axis.title = element_text(size = 13)
  )

# ---- 第五步：加通路标签并导出 ----------------------------------------------
# ggrepel 的 force / padding 来自作者脚本；标签颜色不进图例。
p3 <- p +
  geom_text_repel(
    data = data_label,
    aes(x = xlab, y = NES, label = ID),
    size = 3,
    color = data_label$col,
    force = 20,
    point.padding = 0.5,
    min.segment.length = 0,
    hjust = 1.2,
    segment.color = "grey20",
    segment.size = 0.3,
    segment.alpha = 0.8,
    nudge_y = -0.1
  )

ggsave(
  filename = file.path(root, "preview.png"),
  plot = p3,
  width = 6.2,
  height = 5,
  dpi = 300,
  bg = "white"
)
ggsave(
  filename = "Figure1C.pdf",
  plot = p3,
  width = 6.2,
  height = 5,
  bg = "white"
)
