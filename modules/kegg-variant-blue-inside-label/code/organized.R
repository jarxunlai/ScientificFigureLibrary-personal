# =============================================================================
# KEGG 变体条形图：蓝-黄-红，柱内全黑通路名
# =============================================================================
# 微信文 ggplot 第一版。数据与终图共用 kegg_top15.csv。
# =============================================================================
...
p <- ggplot(data = dt, aes(x = -log10(p.adjust), y = Description, fill = -log10(p.adjust))) +
  ...
  geom_text(size = 4.3, aes(x = 0.05, label = Description), hjust = 0, family = plot_font_family) +
...
ggsave(file.path(out_dir, "kegg_blue_inside_label.png"), p, width = 7.2, height = 6.2, dpi = 300, bg = "white", device = ragg::agg_png)
ggsave(file.path(out_dir, "kegg_blue_inside_label.pdf"), p, width = 7.2, height = 6.2, device = cairo_pdf, bg = "white")
message("wrote kegg_blue_inside_label.png")
